...
public:
    ...
    std::vector<bool> centre;

    std::vector<int> available_IDs;

    std::vector<int> temp_available_IDs;

    float nozzel_angle;

    float flow_sensor_adc;
    ...
    void force_Call_back(const geometry_msgs::WrenchStamped &force);

    bool check_feedback();

    bool clear_protective_stop();

    void tote_corner_points();

    void flow_sensor_call_back(const rosserial_arduino::AdcConstPtr &adc_ptr);
